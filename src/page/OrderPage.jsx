...
const OrderPage = () => {
  const { setReceiptID, orderData, setOrderData, allergyData } =
    useGlobalData();
  ...
  const { top3, appetizer, main_dish, dessert, soldOut_menu, allergy_menu } =
    menuData;
  ...
  const navigateNutrientAnalysisPage = async () => {
    if (orderData.total_price === 0) return;
  ...
  const handleIncrease = (data, name) => {
    setOrderData((prevOrderData) => {
      const newItems = {
        ...prevOrderData.items,
        [name]: prevOrderData.items[name] + 1,
      };
      const newTotalPrice =
        prevOrderData.total_price + getItemPrice(data, name);
      return { items: newItems, total_price: newTotalPrice };
    });
  };
        ...
        <div className='main'>
          <div className='sub-main1'>
            <Category
              title={"TOP3 메뉴"}
              items={top3}
              soldOutMenu={soldOut_menu}
              allergyMenu={allergy_menu}
              addToOrder={addToOrder}
            />
            <Category
              title={"에피타이저"}
          ...
          <div className='sub-main2'>
            ...
            <div className='submit-area'>
              <Button
                className='back'
                buttonName={"이전"}
                onClick={navigateAllergySelectPage}
              />
              <Button
                className='submit'
                buttonName={"주문하기"}
                onClick={() => navigateNutrientAnalysisPage(orderData)}
              />
            </div>
          </div>
        </div>
